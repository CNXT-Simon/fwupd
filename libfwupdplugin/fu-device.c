...
/**
 * fu_device_set_progress_helper:
 * @self: a #FuDevice
 * @progress: a #FuProgress
 *
 * Sets the per-device progress helper which can be used to update the device with a consistent
 * global progressbar with multiple variable speed stages.
 *
 * Since: 1.7.0
 **/
void
fu_device_set_progress_helper(FuDevice *self, FuProgress *progress)
{
	FuDevicePrivate *priv = GET_PRIVATE(self);
	g_return_if_fail(FU_IS_DEVICE(self));
