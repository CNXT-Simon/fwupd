...
fu_realtek_mst_device_use_aux_dev(FuRealtekMstDevice *self, GError **error)
{
	...
	g_udev_enumerator_add_match_sysfs_attr(udev_enumerator, "name", self->dp_aux_dev_name);
...
static gboolean
mst_write_register_multi(FuRealtekMstDevice *self,
			 guint8 address,
			 const guint8 *data,
			 gsize count,
			 GError **error)
...
mst_set_gpio88(FuRealtekMstDevice *self, gboolean level, GError **error)
{
	...
	if (!mst_read_register_indirect(self, REG_GPIO88_CONFIG, &value, error))
		return FALSE;
	if (!mst_write_register_indirect(self, REG_GPIO88_CONFIG, (value & 0xF0) | 1, error))
		return FALSE;
	...
	if (!mst_read_register_indirect(self, REG_GPIO88_VALUE, &value, error))
		return FALSE;
	return mst_write_register_indirect(self,
					   REG_GPIO88_VALUE,
					 ...
					 GError **error)
{
	...
	if (response[0] != 0xca || response[1] != 9) {
		...
		g_debug("unexpected response code %#x, length %d", response[0], response[1]);
	...
	if (info->active_bank > FLASH_BANK_MAX_VALUE) {
		g_debug("unexpected active flash bank value %#x", info->active_bank);
		info->is_enabled = FALSE;
		return TRUE;
	}
...
fu_realtek_mst_device_probe_version(FuDevice *device, GError **error)
{
	...
	if (!fu_realtek_mst_device_get_dual_bank_info(FU_REALTEK_MST_DEVICE(self), &info, error))
		return FALSE;
...
static gboolean
flash_iface_erase_sector(FuRealtekMstDevice *self, guint32 address, GError **error)
{
		  ...
		  GError **error)
{
	...
	for (guint i = 0; i < chunks->len; i++) {
		...
		if (!mst_write_register_multi(self,
					      REG_WRITE_FIFO,
				     ...
				     GError **error)
{
	...
	guint32 base_addr = self->active_bank == FLASH_BANK_USER1 ? FLASH_USER2_ADDR
								  : FLASH_USER1_ADDR;
	guint32 flag_addr = self->active_bank == FLASH_BANK_USER1 ? FLASH_FLAG2_ADDR
								  : FLASH_FLAG1_ADDR;
