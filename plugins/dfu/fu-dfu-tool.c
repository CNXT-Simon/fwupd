...
typedef gboolean (*FuUtilPrivateCb)(FuDfuTool *util, gchar **values, GError **error);
...
static gboolean
fu_dfu_tool_run(FuDfuTool *self, const gchar *command, gchar **values, GError **error)
{
	/* find command */
...
fu_dfu_tool_get_default_device(FuDfuTool *self, GError **error)
{
	...
	/* failed */
	g_set_error_literal(error, FWUPD_ERROR, FWUPD_ERROR_NOT_FOUND, "no DFU devices found");
	return NULL;
}
...
fu_dfu_tool_read_alt(FuDfuTool *self, gchar **values, GError **error)
{
	...
	if (target == NULL) {
		...
		target = fu_dfu_device_get_target_by_alt_setting(device, (guint8)tmp, error);
		if (target == NULL)
			return FALSE;
	}
...
main(int argc, char *argv[])
{
	...
	const GOptionEntry options[] = {
	    ...
	    {"device",
	     ...
	     G_OPTION_ARG_STRING,
	     &self->device_vid_pid,
	     _("Specify Vendor/Product ID(s) of DFU device"),
	     _("VID:PID")},
	    {"transfer-size",
	     't',
	     0,
	     G_OPTION_ARG_STRING,
	     &self->transfer_size,
	     _("Specify the number of bytes per USB transfer"),
	     _("BYTES")},
	    {"force",
	     '\0',
	     0,
	     G_OPTION_ARG_NONE,
	     &self->force,
	     _("Force the action ignoring all warnings"),
	     NULL},
	    {NULL}};
...
#ifdef HAVE_GIO_UNIX
	g_unix_signal_add_full(G_PRIORITY_DEFAULT, SIGINT, fu_dfu_tool_sigint_cb, self, NULL);
#endif

	/* sort by command name */
	g_ptr_array_sort(self->cmd_array, (GCompareFunc)fu_dfu_tool_sort_command_name_cb);
